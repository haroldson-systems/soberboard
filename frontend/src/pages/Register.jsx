...
export default function Register() {
  ...
  const [role, setRole] = useState(searchParams.get("role") === "business" ? "business" : "manager");
  ...
  return (
    <div className="max-w-md mx-auto px-5 py-16" data-testid="register-page">
      ...
      <p className="mt-3 text-[#5C6670]">Free forever for house managers. Business submissions are reviewed before publishing.</p>
      ...
      <form onSubmit={submit} className="space-y-4">
        <div className="grid grid-cols-2 gap-2 rounded-2xl border border-[#EAE5D9] bg-white p-1" data-testid="register-role-selector">
          <button
            type="button"
            onClick={() => setRole("manager")}
            className={`rounded-xl px-3 py-2 text-sm font-semibold transition ${role === "manager" ? "bg-[#2B4C5F] text-white" : "text-[#5C6670] hover:text-[#2D3339]"}`}
          >
            House manager
          </button>
          <button
            ...
            className={`rounded-xl px-3 py-2 text-sm font-semibold transition ${role === "business" ? "bg-[#2B4C5F] text-white" : "text-[#5C6670] hover:text-[#2D3339]"}`}
          >
            Business
          </button>
        </div>
